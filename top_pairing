...
def split_ion_charge(s):
    ...
    s = s.strip()
    # Find a trailing "(<int>+)" at the very end of the string
    m = re.search(r'\((\d+)\+\)\s*$', s)
    if not m:
        # If you prefer to require a charge, return (None, None) instead
        return pd.Series([s if s else None, None])

    charge = int(m.group(1))
    ion = s[:m.start()].rstrip()   # everything before the "(<int>+)"
    return pd.Series([ion if ion else None, charge])


def row_ion_charge_mass(row, ion_col, charge_col, pep):
    ion = row[ion_col]
    charge = row[charge_col]
    if pd.isna(ion) or pd.isna(charge) or len(ion) > 3:
        return np.nan
...
def compute_possible_mass(row, the_target_mass = the_pep.pep_mass):
    ...
    if row['charge1'] is not np.nan and row['charge2'] is not np.nan:
        return row['m/z A'] * row['charge1'] + row['m/z B'] * row['charge2']
...
def conserve_line_table(annot_df, the_pep, mass_threshold = mass_threshold, the_charge = charge, drop_index_na = False):
    ...
    tmpA = annot_df['Interpretation A'].apply(split_ion_charge)
    tmpA.columns = ['ion1','charge1']
    annot_df.loc[:, ['ion1','charge1']] = tmpA.values  # or tmpA

    tmpB = annot_df['Interpretation B'].apply(split_ion_charge)
    tmpB.columns = ['ion2','charge2']
    annot_df.loc[:, ['ion2','charge2']] = tmpB.values
    ...
    if the_charge == 2:
        ...
        annot_df['Mass'] = annot_df.apply(compute_possible_mass, axis=1)
    
    else:
        raise ValueError("Only charges 2 and 3 are supported in conserve_line_table")
    ...
    if drop_index_na:
        annot_df_conserve = annot_df_conserve.dropna(subset=['Index'])
    
    return annot_df_conserve

#annot_df_conserve = conserve_line_table(annot_df, the_pep, mass_threshold=2)
#print(annot_df_conserve[['Index','Interpretation A', 'm/z A', 'm/z_A_correct_1', 'MassDeviation1',
#                         'Interpretation B', 'm/z B',  'm/z_A_correct_2', 'MassDeviation2'
#                         ]])
    


def save_dataframe_to_txt(df, file_path, sequence_name = peptide_name(pepname, charge)):
    """
    Appends a pandas DataFrame to a text file (without overwriting).
    Each write adds a separator line before the new content.
    """
    
    df = df[['Index', 'Interpretation A', 'm/z A', 'm/z_A_correct_1', 'MassDeviation1','charge1',
        'Interpretation B', 'm/z B', 'm/z_A_correct_2', 'MassDeviation2', 'charge2','Mass'
    ]]
    with open(file_path, 'a', encoding='utf-8') as f:  # 'a' means append mode
        f.write(f'\n\n{sequence_name}\n')
        df.to_string(f, index=False)  # write DataFrame content as plain text

#save_dataframe_to_txt(annot_df_conserve, 'conserved_lines_output.txt')

if __name__ == "__main__":
    folder_path = 'data/Top_Correlations_At_Full_Num_Scans_PCov/annotated'
    pattern = re.compile(r'^([A-Za-z]+\d+)_([23])\+\.csv$')

    for fname in os.listdir(folder_path):
        match = pattern.match(fname)
        if match:
            base_name = match.group(1)  # e.g. "ME8"
            charge = int(match.group(2))  # e.g. 2 or 3
            data = os.path.join(folder_path, fname)
            annot_df = pd.read_csv(data)
            ...
            annot_df_conserve = conserve_line_table(
                annot_df,
                the_pep,
            ...
            save_dataframe_to_txt(
                annot_df_conserve,
                'conserved_lines_output.txt',
                sequence_name=peptide_name(pepname, charge)
            )
